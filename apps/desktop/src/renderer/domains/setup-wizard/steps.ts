...
export interface GetSetupWizardStepsOptions {
	...
	/** When true, omit the MetaToken step (already connected: a key is configured or a session exists). */
 ...
 * 其它平台跳过权限步。
 *
 * MetaToken 步紧跟「语言与外观」：登录页与官网授权页都按当前界面语言呈现，
...
export function getSetupWizardSteps(options?: GetSetupWizardStepsOptions): readonly SetupWizardStepId[] {
	...
	// 已接入 MetaToken（有 Key 或已登录）就没什么可操作的，跳过这一步。
