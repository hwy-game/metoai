 ...
 * 目录里也只收「平时静止、被触发才动一下」的装饰件：常驻无限动画的那几枚（燃烧、火把、
 * 星轨、玩手、能源井）每帧都要重绘，窗口空闲时单燃烧一枚就占掉近三成 GPU，已整体移除。
 * 存过这些 id 的用户会被 isOrnamentId 判为无效，回落到这里的默认值。
 */
...
export const ORNAMENT_CATALOG = [
	...
	// 马里奥没有静帧：砖块是按点阵画的 SVG，预览卡直接画一排小的。
