...
export class ActionApprovalBroker implements ActionApprovalRequester {
	...
	request(request: ActionApprovalRequest, signal?: AbortSignal): Promise<ActionApprovalDecision> {
		if (this.webContents.isDestroyed()) {
			...
			return Promise.reject(new ActionError("ACTION_APPROVAL_UNAVAILABLE", "Metoai Desktop 授权界面不可用。"));
		}
	...
	cancelAll(): void {
		...
		for (const pending of this.pending.values()) {
			pending.cancel(new ActionError("ACTION_CANCELLED", "Metoai Desktop 授权请求已取消。"));
		}
		this.pending.clear();
	}
