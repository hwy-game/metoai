 ...
 * apps/im-gateway/. We resolve relative to `process.cwd()`, which is
 * `apps/desktop` when running via `bun run dev` (the same trick
...
export function resolveImGatewayBinary(): ResolvedBinary {
	...
	if (app.isPackaged) {
		...
		// Dev mode: process.cwd() is apps/desktop when launched
