...
> 本文件自 0.5.58 起停止更新。新的发布说明写在仓库根目录的 `.github/release-notes/`，
> 每个版本一个文件（如 `.github/release-notes/v0.5.58.md`），并由 `desktop-release`
> 工作流直接用作 GitHub Release 正文。
>
> 下面的 `[Unreleased]` 段落保留 0.5.49–0.5.57 期间累积的记录：这些条目跨越多个已发布版本，
> 无法再可靠地按版本拆分，因此原样冻结，不再追加。

## [Unreleased]

- Linux x64 正式发布新增 DEB 与 RPM 安装包，并保留 AppImage；三种格式共用同一更新清单，客户端按当前安装方式选择更新包。发布流水线会检查包元数据和文件布局，并分别在 Ubuntu 与 Fedora 容器中完成真实安装后再上传；开发者也可通过独立的 `package:linux:appimage`、`:deb`、`:rpm` 和 `:tar.gz` 脚本只生成指定格式。

- Windows x64 正式发布在现有 Inno 安装器之外新增 MSI 与 ZIP；自动更新仍只使用 Inno，补充制品在 Windows 构建机上解包验证版本目录后再上传。开发者可用 `package:win` 一次生成三种发布格式，也可通过 `package:win:inno`、`:msi`、`:zip` 和 `:portable` 单独打包。

- 手动拖拽侧边栏宽度不再滞后：拖拽途中实时宽度直接写到左栏占位与侧边栏面板两个元素上，不再每帧重渲染整条侧边栏、根布局与当前页面；内容区照常逐帧跟着重排，观感上没有接缝错位。长会话页快拖实测从 p50 57-71ms、38 个长任务共 3000ms 降到 p50 14.3ms、p95 15.2ms、1 个长任务（松手提交那次），宽度跟上每一步指针移动。

- 应用内置 MetaToken 中转站账号：设置里新增「MetaToken」页，登录后能直接看到余额与用量、管理 API Key、用兑换码或在线支付充值，还能一键把账号接入模型配置；首次使用时会先引导登录，也可以跳过、改用自己粘贴的 Key。

- 桌面应用面向用户的产品显示名统一调整为 `Metoai`，覆盖窗口标题、菜单、托盘、欢迎页、聊天与设置文案以及安装包显示名；内部兼容标识、数据目录与协议保持不变。
...
- 打开过的空闲会话不再在主进程无限驻留：正在回复的会话全部保留，最近用过的空闲会话仍可立即切回，更早的空闲会话会在再打开时按原路径恢复。切会话、后台跑完通知和正在进行的回复不受影响。
  某工作区最后一个会话释放后，该工作区的 Coding Agent 运行环境也会一并回收；下一次在这个工作区新建会话会重新装配，首次打开略慢于之前的热复用。

- Agent Team 会话页顶部的成员姓名会在窗口变窄时从末位开始逐个省略并收起，前面的成员不再与后面的成员同时突然丢失文字。

- Agent 设置新增「图片」区：可分别选择文生图与图生图的默认 Provider，Provider 列表随插件注册变化；官方图像生成插件会遵循该选择，并在图片记录中保留实际 Provider 来源。

- 图片及其他媒体生成任务现在写入结构化生命周期日志，包含 Provider、生成模式、请求规格、Job 状态、进度、耗时与分类错误；Prompt、图片内容和本地源文件路径不会进入日志。

- 修复插件图片 Provider 的五分钟生成请求被受管服务 IPC 以超时参数无效拒绝的问题；长时间运行的媒体请求现在可在五分钟上限内正常等待。

- 图片生成工具会主动识别任意语言中的生图请求，包括透明背景图、游戏素材、角色或物品图、贴纸、图标和参考图；网页实现、UI 设计稿、UI 渲染图及 HTML/CSS mockup 仍明确交由代码或设计工具处理。

- Agent Team 成员选择现在点击团队即可进入编辑态：团队内成员显示红色移除标记，其他成员显示加入标记，点击卡片即可直接切换成员。

- Token 用量面板默认展示当前会话总计，并可切换查看当前消息消耗；输入、缓存命中、缓存写入、输出、缓存命中率和模型调用次数会随统计范围同步更新。

- 输入 `/` 打开的命令面板顶部新增“压缩上下文”：图标按当前真实上下文占比绘制圆环；操作会进入可持久化、可删除和可重排的会话队列，当前回复自然完成后才执行，不会中断回复，压缩前后的排队消息仍按顺序处理。

- 上下文用量、压缩资格和压缩执行状态改为由 Runtime 按会话实时推送；新会话不再继承旧会话的上下文状态。

- 会话输入框现在会把文件、图片、目录、能力、场景、连接器和成员的结构化 Token 原样带入发送后的消息气泡；复制粘贴及草稿恢复不再退化为纯文本重解析，历史消息仍可用附件记录恢复文件类型。

- 模型选择器与模型服务设置中的品牌图标改用 Lobe Icons 提供的 SVG；图标随依赖下载并打入应用，无需运行时联网，也不在源码仓库中保存图标文件。

- 新会话页背后的纹理可在「设置 - 外观」里切换：目前提供「网格」（默认，方格纹连同中间那团主色光晕，与此前观感一致）、「流光」（页面正中一团半调网点的光晕，光斑在点阵后缓慢流过并换色）、「涟漪」（与网格同一套排版，纹路换成一层同心圆环）；网格与涟漪的纹路现在都会在页面上下缘之前淡尽，不再被容器边切出一条硬边与「无」（整块背景什么都不画）四档；同时把装饰件与纹理收进「新会话页装饰」这块合并区域，一眼看得出这两项只改新会话页。

- 新会话页输入框上方的工作模式、项目、智能体三枚控件底色由半透明改为实色卡片底，不再透出页面背景。

- 新会话页选中智能体或团队后，欢迎区改为展示该身份：标题上方是智能体头像或团队成员头像组（超过 3 人时末位折成 “+n”），标题与描述换成其名称与简介，切换时以 CSS 动画平滑过渡；未选中时仍是原来的问候语。
...
- 侧边栏改为按产品语义投影会话：使用会话自有工作空间的 Agent Team 会话与普通对话一起展示，选择项目的团队会话展示在对应项目下；团队会话根据首条用户消息自动生成独立标题，左侧使用 Agent Team 图标、右侧展示成员头像，且所有会话行不再显示时间。

- 新会话页同时选择项目与 Agent Team 时，新建团队会话会把该项目固定为工作空间；不选择项目则为每条团队会话分配独立工作空间，不再让同一团队的不同会话共享文件，已有会话不会被后续选择重定向。

- Agent Team 本地存储升级为按 `teams`、`agents`、`workspaces` 分类的可读目录键；升级时一次性迁移旧目录、工作空间、会话路径和旧身份引用，重复工作空间保留到孤儿目录，完成后不再兼容读取旧布局。

- Plugin API 提升至 2.0：插件私有存储改为显式编码的文件接口，并支持多文件 revision 原子提交与一致快照读取；
  旧直写文件会导入初始 revision，旧 1.x 插件在兼容性检查阶段拒绝加载。

- 新增可扩展批量 OCR 基础能力：内置百度开源 PP-OCRv5 本地 Provider 默认可用，插件可按协议注册其它 Provider；
  OCR 选择、远程隐私策略和可用状态进入 Agent 配置，PDF 解析仍由上层插件负责（ADR-0108）。

- Agent Team 随应用提供的初始团队与普通团队使用相同的编辑、保存和删除语义；其 Agent 身份可修改，删除后不会在启动时被隐式恢复。

### Fixed

- 侧边栏项目分组与下方「对话」区域的会话项现在都会在点击后同步提交并持续保持新高亮，不再对选中背景播放颜色过渡、同步测量滚动布局，或在会话打开刚启动时过早回退到旧选中态；导航与恢复只有在高亮完成可见绘制后才会启动，快速连续点击时只处理最后一次选择。

- Agent Team 成员会话现在只由可持久恢复的团队任务层负责自动重试，不再先在 Coding Agent Runtime 内重复重试同一个模型服务错误，缩短 HTTP 500 等故障的失败反馈与恢复时间。

- 团队会话的失败提示改为出现在消息列表的错误组件中，按错误类型给出可理解的提示；输入框上方不再显示原始 IPC 错误。
- 团队成员遇到可重试的模型服务错误时，发送请求会保留任务并按既有策略自动恢复，不再把已经接收的消息误报为发送失败。
- 修复消息列表中的 Markdown HTTP(S) 链接无法打开内置浏览器的问题；普通会话、Agent Team、只读会话和无项目会话现在都使用与活动面板一致的工作空间。

- 修复插件宿主遗漏转发 `useSidebarState`，插件侧边栏状态 Hook 现在与 Plugin SDK 的既有公共 API 保持一致。

- 修复官方服务地址误配为官网域名时，能力市场请求因 308 跳转缺少 CORS 响应头而加载失败；Desktop 现在会把这一历史地址归一化到独立 API 域名。

- 插件重载或停用时会先在 Renderer 边界终止旧 activation 的受管服务调用，迟到的状态轮询不再使用已撤销的 capability session 反复触发主进程错误。
- 修复 Agent Team 在负责人或成员已经输出部分文字、工具调用后遇到 Provider/HTTP 错误时，主会话消息突然消失的问题；失败片段现在会立即保留并写入团队公共记录，重开会话后仍然可见，同时任务继续保持失败或等待状态。
...
- 修复输入框命令面板无法连续使用的问题：点击左下角按钮打开面板后会把焦点交还给输入框，已有文字后输入 `/` 或 `@` 也能触发并筛选对应面板；日期、URL、邮箱等内容仍不会误触发。
- 输入框复制/剪切现在会把 skill、scene、连接器、文件、图片和成员 token 的结构化信息写入剪贴板；同一应用内粘贴优先恢复原始 token，跨应用时再回退到可读的 HTML/纯文本。

- 修复侧边栏会话右键菜单：普通会话和 Agent Team 会话现在都支持删除前二次确认，团队会话同时支持重命名、置顶和打开工作空间。

- 修复新建项目弹窗在中文输入法组合输入时按回车（上屏候选词）被当成确认创建，导致项目名还没打完就被建出去的问题。

- 修复 Agent Team 中止后的工具调用仍显示旋转等待、或把 Runtime 中止竞态误报为失败的问题；未完成工具现在明确收尾为“已取消”，真实工具错误仍保留为失败，重新打开会话也会保持一致。

- 修复负责人停止等待成员后发送“继续”时，已完成或已取消的工具调用被重新渲染为进行中；终态工具流现在会跨越下一轮发送保留，并继续使用原有会话快照清理。

- 插件受管服务请求超时现在会在主进程日志中记录插件、服务、方法、路径、超时和耗时，并向调用方返回可定位的超时错误；日志不会记录请求凭证、请求体或查询参数。
...
- 修复 Desktop 更新本地 Action RPC endpoint 时客户端偶发读到半截 JSON、以及旧实例退出误删新实例 endpoint 的问题；endpoint 现在以原子替换发布，并仅清理仍属于当前实例的文件。

- 修复 Team 在 Ctrl+Enter/停止竞态下的时间线错位：提交中的消息会按时间重新归位，同一键盘事件不会重复提交；停止与消息准入同时发生时仍会保留用户消息，重开后与主会话一致。

- 修复点击停止后成员 Runtime 的正常取消被当成发送失败并传到 IPC/界面的问题；取消现在以正常收尾返回，已写入的用户消息、成员结果和取消状态继续保留，重新打开团队会话时不再丢失内容。

- 修复 Agent Team 点击停止后已显示的负责人部分回复从时间线消失的问题；中止回合现在作为终态保留，迟到的流事件不会重新打开回合，取消后的会话列表仍保留已有内容。

- 普通会话与 Agent Team 发送入口现在统一区分两种意图：Enter 将消息排到当前 Turn 结束后处理，Ctrl+Enter（可在快捷键设置中配置）使用 steer 语义在安全边界优先处理；团队成员的排队请求保留独立 WorkItem/Attempt 身份，连续发送、停止和重新打开会话时不再合并或重复呈现。

- Agent Team 成员页面现在会把成员 Runtime 的持久化工具历史与同一工具调用的实时流归并为一个稳定消息；运行中写入快照、切换成员以及重新打开会话时不再短暂或持续显示两份相同回合。

- 打开 Agent Team 会话时，侧边栏现在只高亮当前团队会话，不再同时保留上一普通会话的选中背景；默认对话区与项目分组使用相同的单选规则。

- Agent Team 现在会保留 Runtime 模型选择阶段抛出的结构化 Provider 错误；重开会话后若保存的模型已从目录移除，发送会明确失败并提示模型不可用，不再被降级成可恢复的“已中断”后向 IPC 和界面误报成功。多成员请求仍会等待其它成员完成并保留其结果。

- 项目展开后的 Agent Team、普通会话及状态图标现在共享同一层级缩进，团队会话不再与项目标题错误对齐。

- Agent Team 任一成员执行期间，默认对话区与项目分组中的团队会话都会用旋转加载图标替代左侧 Team 图标；全部成员结束、失败或停止后自动恢复 Team 图标，切走会话页面也不会丢失运行状态。

- 新会话页切换普通对话、单个智能体或 Agent Team 时现在共用当前可见草稿，正文与附件不再消失；团队成员 `@` Token 离开所属团队后保留为普通文字且不携带成员路由，切回原团队时若文字未被改动则恢复为成员 Token。

- Agent Team 会话中的设置按钮现在紧跟成员胶囊靠左排列，不再被空白区域推到窗口右侧。

- Work 模式的工具调用现在优先展示模型返回的用户可读 `description`，工具没有提供描述时才回退到语义化工具名；文件路径、命令等调用对象继续作为次要上下文展示。

- Agent Team 用户消息现在始终出现在主会话，并且只在编辑器成员 Token 明确指定的成员会话中出现；正文继续按 Markdown 渲染，已发送的成员提及按结构化 metadata 复用输入框的头像 Token，手敲的普通 `@handle` 不再被字符串匹配误判。

- 修复 Agent Team 停止时成员完成通知、孤儿运行任务、自动重试或外部条件变化仍可能重新唤起成员与负责人的问题；停止现在会并行中断 Runtime、以持久化屏障取消全部未终结任务，并拒绝跨越停止边界的迟到委派，重新打开会话也不会恢复旧任务，只有新的用户消息可以开始新一轮执行。
- 修复 Agent Team 成员完成任务后，其执行槽仍被负责人续跑占用的问题；负责人再次向同一成员委派任务时，新任务现在会立即从等待状态开始执行，不再与完成通知形成循环等待。
- 修复 Agent Team 负责人调用成员后，成员私有执行历史在运行中或重新打开会话时被再次合并进公共时间线、导致连续重复消息和前后呈现不一致的问题；团队主会话现在只以协调 Conversation 为消息来源，按发布记录恢复已完成工具结果，并在重开活动会话时重放负责人/成员的工具生命周期与持久化运行状态，避免工具消失或会话误显示为空闲。

- 插件工作区导航现在会保留宿主解析后的全彩图标地址；受管服务启动期间暂不可用的 MCP 贡献会安静暂缓，服务就绪后再发布，避免品牌图标被错误渲染为单色遮罩并反复记录未就绪告警。
...
- 智能体编辑页的保存操作现在位于吸顶标签栏，浏览较长的能力列表时无需滚动到底部；跨团队影响确认、保存结果和错误反馈也会显示在同一顶部区域。

- 修复 Agent Team 成员回复卡片的头像与名称因右侧会话入口撑高标题行而上下错位的问题；头像、名称与状态现在在同一标题行内垂直居中。

- Agent Team 能力设置现在解析插件本地化名称，并按来源插件归组其 Skill；选择插件会自动包含其贡献的 Skill，选择单个插件 Skill 也会同时包含所属插件，避免界面显示 `%plugin.name%` 或保存出不可用的孤立 Skill。

- 负责人通过 `team_send_message` 委派成员时，成员活动现在按来源 `toolCallId` 固定渲染在对应工具位置；成员卡片不再作为负责人消息外的独立时间线条目，也不会随消息级或 Work 阶段折叠消失；缺少关联字段的旧会话继续使用兼容的外层卡片。

- Agent Team 中由用户直接选择或 `@` 的成员现在像负责人一样显示完整普通消息气泡，包括等待、流式和完成状态；只有负责人发起的成员委派继续使用紧凑回复卡片，不再出现成员已回复但聚合会话不可见的问题。

- Agent Team 聚合会话现在以最高 240px、随内容自适应的摘要卡展示成员的 thinking、工具阶段和回复状态，并通过稳定的 request/source turn 关联避免成员完成后卡片丢失或错位；成员乱序完成时摘要卡仍保持原委派顺序，点击卡片可进入对应成员会话。

- 统一新会话发送团队消息时，先保留会话 ID 和发送快照并立即切换到团队会话页，再由目标页在首帧后创建 Team 会话记录并发送，不再等待 Runtime 初始化或负责人处理完成才跳转；发送期间输入框保持可编辑，后续输入与已发送请求使用独立快照，不会被清空或修改；StrictMode 重放和空初始化快照不会清掉在途请求，乐观消息、流式消息与持久化消息现在共用同一渲染身份，真实 Snapshot 到达时不再重挂载可见消息列表。

- Agent Team 公开时间线不再把仅路由给其他成员的内部指令显示为第二条负责人回复；同一负责人回合的工具步骤与最终文本合并在一个气泡中，工具完成后保持终态。

- 修复 Agent Team 消息列表滚动抖动：滚动跟随在到达底部后停止空转，连续尺寸修正合并到单帧处理，并为向上滚动预留历史消息缓冲；重新打开已有会话时不再先使用零缓冲首屏再切换布局，避免历史消息批量替换；同时使用 Virtuoso 状态快照恢复已测量行高，避免动态高度消息在再次挂载或滚动时反复改变滚动条比例。

- 能力市场安装流程现在区分检查来源、安装、刷新本地状态和激活阶段；权限弹窗直接使用安装结果，确认配置通过一次事务写入，避免整页市场刷新造成的额外等待。

- 能力市场详情抽屉壳同步打开，Markdown/Shiki 正文按需加载；关闭时立即让遮罩穿透，退出动画完成后再卸载内容。

- 插件更新遇到权限或命令声明变化时会先停在确认弹窗：新增项默认勾选、移除项明确标记，用户确认后才切换并激活新版本，避免更新后运行时才暴露缺少权限。

- 市场受管 MCP 改为宿主拉起本地 HTTP 服务并直连 Streamable HTTP，不再经 stdio 桥接；登录改为上游二维码与状态端点，能力详情提供独立的连接设置页，代理等运行时参数不再混入通用 MCP 编辑器。

- 修复小红书扫码登录打开时，详情状态检查与二维码请求因共享全局取消信号而互相中止的问题；二维码取消现在按单次请求隔离，预期取消也不再记录为网络故障。

- Agent Team 新建对话现在会先进入可交互界面并更新侧栏选择态，首帧后只建立轻量协调记录；负责人 Runtime 优先、其余成员并行后台预热。用户可在预热完成前提交消息，消息与负责人处理状态会立即显示，团队目录和成员资料刷新不再阻塞关键路径。

- Agent Team 首条消息现在只等待本次实际寻址成员的 Runtime 就绪，其他成员继续后台预热；待回复消息会依次显示“团队正在加载”和“等待模型响应”，流式输出开始后恢复普通处理状态；成员历史中的重复回复不会再复用同一渲染 key，避免首次响应前后的无效等待和列表抖动。

- 统一新会话的首条团队消息现在会把执行模式随会话记录一次性固定，并将首帧快照与发送请求交接给团队会话页；路由切换后发送失败仍能在当前会话中恢复，不再依赖已卸载的新会话页。

- 修复受管服务插件增删自有模型 Provider 后当前窗口的模型目录未立即刷新，导致插件在启动期重新发布的模型仍需手动刷新或再次应用才能出现。

- 修复受管服务健康检查先于业务模型目录完成时过早汇报 `ready`，导致 CPA 启动期间用空目录覆盖已保存 Provider；现在插件可声明语义 readiness，宿主在模型目录可用前保持 `starting`。

- 修复远程会话执行失败时 Runtime 返回的结构化错误回执被忽略，导致客户端只能看到笼统的执行失败；现在会保留认证、权限和重试信息供远程端正确映射。

- 修复 Agent Team 聚合时间线同时展示协调消息和成员 Runtime 入站消息，导致同一条用户消息重复显示的问题；协调 Conversation 现在作为公共用户消息的唯一来源。

- 修复 Agent Team 成员 Runtime 返回认证或其他终止失败时被静默当作中断等待的问题；现在会保留结构化失败状态、显示错误并将不可重试的认证问题标记为需处理。

- 修复远程控制连接可能接受旧连接的 `hello_ack`、错误显示为已连接的问题；现在会校验确认包所属的连接 ID，并忽略过期确认。
...
- 兼容未提供 secrets 变更通知的旧版 preload，插件仍可正常加载并使用 secrets 读写能力。

- At 面板在尚未建立工作区时不再向文件系统 IPC 发送空目录，避免 `Invalid dirPath` 未捕获错误。

- 插件开发服务器固定使用宿主公布的本机回环 origin，避免 Module Federation 在 Windows 上推断出不同主机名后无法加载虚拟入口。

- 修复 Agent Team 消息列表在上下滚动并逐步测量消息高度时，滚动区域不断被撑高并在底部留下大量空白的问题。

- Agent Team 成员完成负责人委派的任务后，会通过 Runtime 续跑上下文通知任务发起者；通知携带稳定的任务、成员和结果消息 ID，避免负责人继续依据旧状态判断任务仍在执行。

- Agent Team 时间线现在始终保留协调会话中的用户消息，即使成员会话投影已加载；成员历史中的重复用户消息会按会话回合去重。

### Added

- 插件 AI 能力新增真正的流式完成：`ctx.ai.stream()` 将主进程 Provider 的文本增量实时送到插件 UI，沿用
  `ai.complete` 权限、模型解析、凭据与最终 usage 合同，并支持跨 IPC 取消。

- 新会话页统一承载普通会话与 Agent Team 会话；默认使用普通会话，选择团队后切换为团队输入链路，团队侧栏的新建入口复用同一页面并预选对应团队。项目与团队选择器共享可组合的 Picker 组件，团队草稿在真正发送时才创建协调会话记录。
...
- 新增系统插件 **iOS 模拟器**：活动面板按 cwd 在 macOS 的 Xcode / SwiftPM 工程中上栏，内嵌 baguette 自带的模拟器
  ...
  站内跳转（打开侧栏视图、返回列表）不会被面板拽回设备页——首次就绪时导航一次，之后交给用户。直接获得实时画面、点击与拖动，以及机型外框、旋转、摇一摇、状态栏覆盖、模拟定位、摄像头、无障碍检查器、
  ...
  内容区撑出横向滚动；右缘悬浮工具条与
  返回列表链接、以及顶栏最宽的 capture-size 一簇按各自宽度阈值整簇收起（面板拉宽后自动回来），避免被
  裁成半个按钮；折叠菜单的下拉浮层改以居中的工具栏为定位基准，不再因向左展开而被面板边缘裁掉；
  收起右缘工具条的同时一并去掉为它预留的 58px 留白，使设备在面板内保持居中；滚动条只
  隐藏外观、不改内容区 `overflow`，因此滚动仍然可用。面板也刻意不自绘画面：`serve` 对 WebSocket 另有 Origin 白名单，只放行无 Origin
...
- 插件网络桥会在进入 Capability/IPC 边界前移除顶层可选字段的显式 `undefined`，避免合法 GET 请求因
  `body: undefined` 被 JSON 输入校验拒绝；已定义的 JSON 与 multipart body 保持原样。
- 插件设置页的密码输入不再因安全脱敏目录刷新而逐字符清空；密码完成输入后一次性写入系统凭据库，并向已激活的
  Renderer 与 Agent 插件运行时发布完整有效设置，插件无需重载即可处理新凭据。输入框同时改用统一高对比度控件，
  已保存的密码只显示安全状态，不回显明文。
...
- 消息列表拆分为只读基座与普通会话配方：编辑、分叉、删除及运行 Footer 由外部组合；新增消息、内容块和 Markdown 局部渲染扩展。展开状态和卡片缓存在列表间隔离，跨会话确认操作不再把结果回写新会话。

- Agent Team 输入框的成员路由头像现在为每个成员显示身份徽标，负责人使用负责人标签，预设角色和自定义成员使用对应身份；徽标不拦截鼠标事件，长身份会截断并保留完整提示。
- Agent Team 设置页重新整理为紧凑的标题栏、成员导航和配置内容三段布局：减少顶部空白，团队名称成为唯一标题标识，移除无意义的默认字母头像，成员选中态与负责人徽标更清晰；团队结构与成员能力共用一个保存入口，删除操作与保存操作保持同一行，成员头像、身份信息和能力配置按卡片分组。
- 「设置 → 更多选项」收拢所有不占侧边栏的插件页面，并**在设置壳内**打开它们：两层侧栏保持可见，切换另一个插件页面
  只需一次点击，不必退回设置再展开一次。侧栏该项右侧新增展开按钮，就地列出这些页面；点击行本身仍进入列表页。
  浏览器操作、构建 Apple App、ComfyUI 视频生成、内容创作配置四个内置插件的配置页改为不占侧边栏导航位。

- 删除「设置 → 工具配置」页：插件配置改由插件自己的工作区页面承载（ADR-0105），设置页只保留应用级配置。原先该页
  唯一的内置项「图片处理」并入「设置 → Agent 配置 → 运行时」。已配置的插件普通设置在升级时自动迁入各插件私有存储，
  已保存的插件密钥留在系统加密凭据库中，位置不变。「ComfyUI 视频生成」与「内容创作」改为在侧边栏自己的配置页里
  设置，前者带连通性自检，后者按服务商分组并且不再回显已保存的 API Key。
...
- 新会话页的团队选择器改为展示与侧边栏一致的成员头像栈；团队菜单项与选中后的触发器复用同一个头像组件和成员映射规则，成员较多时统一显示前三个头像与剩余数量。
