...
> 本包属于 **Frontend Apps Team**，同组包：`apps/admin`
...
**设计、实现或审查 Desktop 用户可见 UI 或文案时，必须先完整阅读 [`docs/user-language-ui.md`](./docs/user-language-ui.md)。**
查看条件包括新增或调整页面、导航与信息分组、视觉层级、操作流程、表单、状态与错误反馈、可访问性，以及用户可见术语和文案（含 i18n catalog、主进程原生菜单、通知和对话框），不以是否修改 `.tsx` / `.css` 为限。
该文档规定如何从用户目标和心智模型组织界面；实现前完成其中的设计推演。视觉实现仍遵循 `DESIGN.md`，文案国际化仍遵循下文 i18n 规则。
...
**新增或修改由用户操作触发的异步加载、资源创建、Runtime/进程初始化、索引或大数据恢复流程时，必须完整阅读并遵守 [`docs/ui-first-async-work.md`](./docs/ui-first-async-work.md)。**
默认把“可见完成”和“功能就绪”拆成两个里程碑：先提交路由、选择态、页面壳和可恢复草稿，完成可见绘制后再启动非关键准备。只有安全、身份或页面有效性所必需的检查可以阻塞导航；测试必须证明 UI 状态先于昂贵工作，而不只断言最终结果。
...
- **theme-ui 视图不绑全局键**；快捷键在 desktop 的 hook/model 里用 `useShortcutScope` 注册。
...
desktop 的文本日志统一由 `src/main/logger.ts` 管理。新增或修改日志时，优先使用这里的入口，不要直接手写 `appendFileSync`、`console.log` 文件重定向，或重新实现独立轮转逻辑。
