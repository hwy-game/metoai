...
export function formatPlaywrightAttachFailure(details: PlaywrightAttachFailureDetails): string {
  ...
  if ((details.devtoolsTargetCount ?? 0) > 0) {
    hints.push(
      `CDP currently exposes ${details.devtoolsTargetCount} DevTools target(s). If the output stops after <ws connected>, close stale DevTools windows and retry; keep the Metoai Desktop window open.`,
    );
  }
